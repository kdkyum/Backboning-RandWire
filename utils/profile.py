...
def profile(model, inputs, custom_ops=None, verbose=True):
    ...
    def add_hooks(m):
        if len(list(m.children())) > 0 and not type(m) in custom_ops:  # skip for non-leaf module
            return
    ...
    for m in model.modules():
        if len(list(m.children())) > 0 and not type(m) in custom_ops:  # skip for non-leaf module
            continue
        if type(m) in custom_ops:  # skip for non-leaf module
            non_leaf_ops += m.total_ops
        elif len(list(m.children())) == 0:
            total_ops += m.total_ops
            total_params += m.total_params

    print('leaf_ops:', total_ops)
    print('non_leaf_ops:', non_leaf_ops)
    total_ops += non_leaf_ops
